#!/usr/bin/env python
"""Compare Op Semantics reports across models."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compare Op Semantics reports.")
...
def main() -> int:
    ...
    if not reports:
        print("[error] no op semantics reports found", file=sys.stderr)
        return 1
    comparison = compare_op_semantics(reports)
    out_json = root / "reports" / "op_semantics_compare" / "summary.json"
    out_md = root / "reports" / "op_semantics_compare" / "summary.md"
    write_json(comparison, out_json)
    write_markdown(comparison_to_markdown(comparison), out_md)
    print(f"[op-semantics-compare] models={comparison['num_models']}")
    print(f"[op-semantics-compare] json={out_json}")
    print(f"[op-semantics-compare] markdown={out_md}")
    return 0
