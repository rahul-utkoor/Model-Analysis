#!/usr/bin/env python
"""Compare symbolic pruning plan sets across models."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compare symbolic pruning plan sets.")
...
def main() -> int:
    ...
    if not reports:
        print("[error] no pruning plan reports found", file=sys.stderr)
        return 1
    comparison = compare_pruning_plan_sets(reports)
    out_json = root / "reports" / "pruning_plan_compare" / "summary.json"
    out_md = root / "reports" / "pruning_plan_compare" / "summary.md"
    write_json(comparison, out_json)
    write_markdown(comparison_to_markdown(comparison), out_md)
    print(f"[pruning-plan-compare] models={comparison['num_models']}")
    print(f"[pruning-plan-compare] json={out_json}")
    print(f"[pruning-plan-compare] markdown={out_md}")
    return 0
